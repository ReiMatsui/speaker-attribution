...
機能:
  ...
  - 声紋プロファイル方式の話者特定（登録不要で自動補正）。判定は2経路のみ:
      ...
      しきい値は3層: モデル別既定値 → セッション自動校正(部屋の分布、厳しくする方向のみ)
      → 人物別しきい値(本人の一致sim中央値-0.12、新声の巻き取り防止)。
...
class VoiceProfiles:
    ...
    しきい値は3層構造（すべて「厳しくする方向にのみ」働き、最悪でも既定値の挙動に戻る）:
      1. モデル別既定値(DEFAULTS) 2. セッション自動校正(部屋のラベル内/ラベル間類似の中間)
      3. 人物別しきい値(その人物の一致sim中央値-0.12 = 新規性検出。中途半端な類似の
         新しい声を既存人物に巻き取らない)
    ...
    DEFAULTS = {"resemblyzer": (0.75, 0.78, 0.62), "ecapa": (0.35, 0.40, 0.30),
