...
- `offshelf/live_soniox.py` … 1マイク＋Soniox（日本語・話者分離）リアルタイム議事録ツール。**これが本命。**
  ...
  - 機能（2026-06-10実装、モックサーバで検証済み・実機ライブ確認は未）:
    ...
    ⑤-b 自動登録（2026-06-10夕、登録不要の自動補正）: 未知の声が3発話一貫(全ペア類似>=0.62)で
       匿名「人物N」として凍結登録→以降は声で追跡、ラベル入れ替わりを自動補正。
       対応の変更は強い証拠(類似>=0.75かつ2位差>=0.10)なら即時、弱い証拠は2回連続で確定。
       実名(1=松井)のみvoices.jsonに永続化、匿名はセッション限り。--vp-no-autoで無効化。
       検証(AMI実音声・40会議シミュレーション・重なり後50%入れ替わりの誤りモデル):
       素ラベル66.3% / 自動登録81.1% / 手動登録(oracle)88.3%。Soniox完璧時の害は-2.4%。
       声紋実測: 発話単発の同一人物類似は平均0.70(5%点0.53)とノイジー、3発話プロファイルで
       平均0.81に改善。floor=0.65が最適(本人取りこぼし3.8%/他人誤通過3.7%)。
